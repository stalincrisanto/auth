...
import { findUserName } from "../db/prisma/userCredentials";
...
export const configuration: Configuration = {
  ...
  async findAccount(ctx, id) {
    try {
      console.log("entro a findAccount")
      console.log("id =>", id)
      const account = await findUserName(id);
      ...
      return (
        account && {
          accountId: id,
          name: "Ela Dmin",
          claims: async function (use /* id_token, userinfo */, scope, claims) {
            // console.log(`claims(${use}, ${scope}, ${JSON.stringify(claims)})`);
            if (!scope) return undefined;
            const openid = { sub: id };
            const email = {
  ...
  clients: [
    {
      ...
      redirect_uris: [
        "http://localhost:3005/cb",
        "https://jwt.io/",
        "http://localhost:3000/api/auth/callback/app",
        "http://localhost:3000",
        "http://localhost:3000/api/auth/callback/mediaValue",
